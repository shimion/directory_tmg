      ...
      if($i < $count)
      {
        ...
        if($image){
          switch($itemMetaValue['itemType']){
            case 'image':
              if($display == 'fancybox'){
                $result .= '<div class="tile" style="display: none"><div class="tileWrap" style="overflow: hidden">' . AitImageResizer::resize($image, array('w' => 305, 'h' => 305)) .'</div><span class="gridgallery-icon"><a class="tileImage" href="'.$image.'">';
              } else {
                $result .= '<div class="tile" style="display: none"><div class="tileWrap" style="overflow: hidden">'.AitImageResizer::resize($image, array('w' => 305, 'h' => 305)) .'</div><span class="gridgallery-icon"><a class="tileImage" href="'.get_permalink( $item->ID ).'">';
                // style="background: url(\''.TIMTHUMB_URL . '?src='.$image.'&amp;w='.$width.'&amp;h='.$height.$bigImageAlignment.'\') no-repeat center center"
              }
            break;
            case 'website':
                $result .= '<div class="tile" style="display: none"><div class="tileWrap" style="overflow: hidden">'.AitImageResizer::resize($image, array('w' => 305, 'h' => 305)) .'</div><span class="gridgallery-icon"><a class="tileImage" href="'.$itemMetaValue['websiteLink'].'">';
            break;
            case 'video':
              if($display == 'fancybox'){
                $result .= '<div class="tile" style="display: none"><div class="tileWrap" style="overflow: hidden">'.AitImageResizer::resize($image, array('w' => 305, 'h' => 305)) . '</div><span class="gridgallery-icon"><a class="tileImage" href="'.$itemMetaValue['videoLink'].'">';
              } else {
                $result .= '<div class="tile" style="display: none"><div class="tileWrap" style="overflow: hidden">'.AitImageResizer::resize($image, array('w' => 305, 'h' => 305)) . '</div><span class="gridgallery-icon"><a class="tileImage" href="'.get_permalink( $item->ID ).'">';
              }
            break;
