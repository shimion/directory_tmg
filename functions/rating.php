 ...
 <div class="ratings">
					<?php for($i = 1; $i <= 5; $i++){
						...
						if(isset($aitThemeOptions->rating->$nameEnable)){
							...
							$nameTitleSchool   = 'rating'.$i.$i.'Title';
							$nameTitleBusinesses = 'rating'.$i.$i.$i.'Title';	?>
...
if (in_array($term[0]->slug, $schoollisting)){ ?>
	<?php echo $aitThemeOptions->rating->$nameTitleSchool;
}elseif(in_array($term[0]->slug, $businesseslisting)){
	echo $aitThemeOptions->rating->$nameTitleBusinesses;
}else{
	echo $aitThemeOptions->rating->$nameTitle;
} 
