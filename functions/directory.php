...
function getItems($category = 0, $location = 0, $search = '', $radius = array()) {
	...
	$params = array(
		...
		'nopaging'	=>  true,
		'post_status'	=> 'publish',
		'update_post_term_cache' => false , //* post terms
   		'update_post_meta_cache' => false, //* post meta
   		'no_found_rows' => true  // post pagination
	);
...
function getItemsAjax() {
	...
	foreach ($items as $item) {
		...
		$item->timthumbUrlContent = (isset($item->thumbnailDir)) ? AitImageResizer::resize($item->thumbnailDir, array('w' => 100, 'h' => 100)) : '';
	}
