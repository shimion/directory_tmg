...
} else {  if(geoloc){ radius.push(parseInt({ifset $geolocationRadius}{$geolocationRadius}{else}100{/ifset}));
...
function( response ) {	  if((!reset) && (!geoloc) && ($('#directory-search').data('interactive') == 'yes')){
  ...
  if((!reset) && (!geoloc)){ 	generateContent(response); } {/if}
...
infobox.close(); mapDiv.gmap3({ clear: { } }); map.setZoom({!$themeOptions->directory->setZoomIfOne}); 
...
options: { icon: dataRaw[key].marker, shadow: "{!$themeOptions->directoryMap->mapMarkerImageShadow}", },
...
if(geoloc){
 ...
 options: { animation: google.maps.Animation.DROP, zIndex: 1000, icon: "{$themeImgUrl}/geolocation-pin.png" } } });	}	
...
var title;
if(length == 0){ title = $('<header class="entry-header"><h1 class="entry-title"><span>{__ 'No result found'}</span></h1></header>');
} else { title = $('<header class="entry-header"><h1 class="entry-title"><span>{__ 'Search result'}</span></h1></header>'); }
var html;
